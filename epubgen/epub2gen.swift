...
class epub2gen: epubgen {
    ...
    let dispatchQueue = DispatchQueueFactory.CreateDispatchQueue(component: "\(epub2gen.self)")
    ...
    fileprivate func generatePackage() {
        ...
        dispatchQueue.async {
            ...
            let tocNcxId = ContentOpf.createItemUUID()
            epub.contentOpf.manifest.addItem(id: tocNcxId, href: epub.tocNcx.filename, mediaType: MimeTypes.ncx)
            epub.contentOpf.spine.tocNcxManifestId = tocNcxId
            
            do {
                self.workingDirectory = try TemporaryDirectory.create()
                let writer = Epub2Writer(epub: epub, destination: self.workingDirectory!.url, filesToInclude: self.filesToInclude)
    ...
    fileprivate func processMetadata(config: Config, epub: Epub2) {
        ...
        epub.tocNcx.dtbUid = config.identifier
        epub.tocNcx.title = config.title
        epub.tocNcx.author = config.author
    }
     ...
     */
    fileprivate func processFiles(files: [URL], tocEntries: [String: String], epub: Epub2) {
        ...
        var isFirstXhtmlOrMarkdownFile = true
        for fileURL in sortedFiles {
            ...
            if fileExtension == FileExtensions.xhtml {
                ...
                if isFirstXhtmlOrMarkdownFile {
                    epub.contentOpf.guide.addReference(href: fullFileName, type: .title, title: tocEntries[fullFileName] ?? fileName)
                    isFirstXhtmlOrMarkdownFile = false
                }
                
                if let tocEntry = tocEntries[fullFileName] {
                    epub.tocNcx.addNavPoint(src: fullFileName, title: tocEntry)
                }
            }
            ...
            if fileExtension == FileExtensions.md {
                ...
                do {
                    ...
                    let xhtmlDocument = XhtmlDocumentEpub2(filename: "\(fileName).xhtml", title: xhtmlTitle, styleHref: config.style, body: html)
                    Output.printStdOut(message: "Converted \(fullFileName)")
                    epub.add(xhtmlDocument: xhtmlDocument, tocTitle: tocEntries[fullFileName])
                    
                    if isFirstXhtmlOrMarkdownFile {
                        epub.contentOpf.guide.addReference(href: xhtmlDocument.filename, type: .title, title: tocEntries[fullFileName] ?? fileName)
                        isFirstXhtmlOrMarkdownFile = false
                    }
                } catch let error {
